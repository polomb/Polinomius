...
public:
    ...
    ForwardList(std::size_t n, const T& elem = T{}) {
        if (n == 0) {
            return;
        }
        first = new Node(elem);
    ...
    ForwardList(const ForwardList& other) : first(nullptr), listSize(0) {
    ...
    T& operator[](std::size_t index) {
        if (index >= listSize) {
            throw std::out_of_range("ForwardList::operator[] index out of range");
        }
        Node* cur = first;
    ...
    T& at(std::size_t index) {
        if (index >= listSize) {
            throw std::out_of_range("ForwardList::at index out of range");
        }
        Node* cur = first;
        for (std::size_t i = 0; i < index; ++i) {
            cur = cur->next;
    ...
    T& front() {
        if (empty()) {
            throw std::out_of_range("ForwardList::front");
        }
        return first->data;
    }
