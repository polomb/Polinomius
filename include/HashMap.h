...
private:
    ...
    ForwardList<Pair>* buckets = nullptr;
    ...
    void rehash(std::size_t newCount) {
        ForwardList<Pair>* newBuckets = new ForwardList<Pair>[newCount];
...
public:
    ...
    explicit HashMap(std::size_t count = DEFAULT_BUCKET_COUNT, const Hash& hash = Hash{})
        : buckets(new ForwardList<Pair>[count]), bucketCount(count), elementCount(0), hasher(hash) {}

    HashMap(const HashMap& other)
        : buckets(new ForwardList<Pair>[other.bucketCount]),
    ...
    HashMap& operator=(const HashMap& other) {
        ...
        elementCount = other.elementCount;
        hasher = other.hasher;
        buckets = new ForwardList<Pair>[bucketCount];
        for (std::size_t i = 0; i < bucketCount; ++i) {
            buckets[i] = other.buckets[i];
        }
        return *this;
    ...
    class Iterator {
    private:
        ForwardList<Pair>* buckets;
        std::size_t bucketCount;
        std::size_t bucketIdx;
        typename ForwardList<Pair>::Iterator listIt;
    ...
    public:
        ...
        Iterator(ForwardList<Pair>* buckets, std::size_t bucketCount,
                 std::size_t bucketIdx,
                 typename ForwardList<Pair>::Iterator listIt)
    ...
    Iterator end() {
        return Iterator(buckets, bucketCount, bucketCount,
                        typename ForwardList<Pair>::Iterator{});
    }
};
